...
void CEntity::OnLoop() {
    ...
    //We're not Moving
    if(MoveLeft == false && MoveRight == false && Type == ENTITY_TYPE_PLAYER) {
        StopMove();
    }
...
bool CEntity::PosValidEntity(CEntity* Entity, int NewX, int NewY) {
    if(this != Entity && Entity != NULL && Entity->Dead == false &&
        Entity->Flags ^ ENTITY_FLAG_MAPONLY && Entity->Flags ^ ENTITY_FLAG_GHOST &&
...
void CEntity::Shoot(int targetX, int targetY) {
}
